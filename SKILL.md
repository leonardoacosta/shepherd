---
name: shepherd
description: "Control Shepherd, a terminal multiplexer for coding agents. Use only when the user explicitly mentions Shepherd or asks to use Shepherd to inspect or control panes, tabs, workspaces, commands, or another agent. Do not use merely because a task could benefit from a background terminal, delegation, or parallel work. Requires SHEPHERD_ENV=1."
---

# Shepherd

Shepherd organizes terminals into workspaces, tabs, and panes, recognizes coding agents running inside panes, and exposes the current session through the `shepherd` CLI.

Before issuing any control command, verify that this agent is running inside a Shepherd-managed pane:

```bash
test "${SHEPHERD_ENV:-}" = 1
```

If the check fails, say that you are not running inside Shepherd and stop. Do not inspect or control the focused Shepherd session from outside Shepherd.

When the check passes, the `shepherd` binary in `PATH` talks to the current session. Use it to inspect neighboring work, create terminal layout, start agents and commands, read output, and wait for state changes.
...
Do not run bare `shepherd` for discovery; it launches or attaches the TUI. Do not probe a mutating nested command by omitting arguments. Commands such as `shepherd workspace create` are valid with defaults and will execute.
...
A pane exists whether or not it contains an agent. `agent start` requires an existing available shell pane and never creates, splits, or moves layout. Use pane commands for ordinary processes. Use agent commands when Shepherd must validate agent identity or interpret `idle`, `working`, `blocked`, `done`, and `unknown` lifecycle states.
...
`idle` means the agent is ready for input and its tab has been seen in the focused Shepherd UI. `done` is the same underlying idle state after unseen background work finishes. Focusing the tab or targeting the pane or agent with a focus command marks it seen. CLI reads do not mark it seen. `blocked` means Shepherd recognized an approval or question UI. `unknown` means an agent is present but Shepherd cannot classify it confidently; it does not prove completion.
...
Shepherd injects the caller's context into each managed pane:

```bash
printf '%s\n' "$SHEPHERD_WORKSPACE_ID" "$SHEPHERD_TAB_ID" "$SHEPHERD_PANE_ID"
```
...
shepherd tab list --workspace "$SHEPHERD_WORKSPACE_ID"
shepherd pane current --current
shepherd pane list --workspace "$SHEPHERD_WORKSPACE_ID"
shepherd agent list
```
...
```bash
shepherd pane layout --pane "$SHEPHERD_PANE_ID"
```
...
```bash
shepherd pane split --current --direction right --cwd "$PWD" --no-focus
```
...
```bash
shepherd agent start reviewer --kind codex --pane <returned-pane-id>
```

Use the kind requested by the user. Run `shepherd agent` to inspect the installed kind list and options. Pass native agent arguments only after `--`:

```bash
shepherd agent start reviewer --kind codex --pane <returned-pane-id> -- <agent-args...>
```

`agent start` returns only after Shepherd detects the expected agent in the same pane and considers it ready for interactive input. It defaults to a 30-second startup timeout.

Submit work through the agent surface:

```bash
shepherd agent prompt reviewer "Review the current diff and report only actionable findings." --wait --timeout 120000
```
...
```bash
shepherd agent send-keys reviewer esc
shepherd agent send-keys reviewer ctrl+c
```

Shepherd validates all keys before writing any bytes. Read the result through the resolved agent:

```bash
shepherd agent get reviewer
shepherd agent read reviewer --source recent-unwrapped --lines 120
```
...
```bash
shepherd pane run <returned-pane-id> "just test"
shepherd pane wait-output <returned-pane-id> --match "test result" --timeout 120000
shepherd pane read <returned-pane-id> --source recent-unwrapped --lines 120
```
...
`--lines` asks Shepherd for more rows from the pane's available screen and host scrollback. If increasing it does not reveal more of a completed response, the pane is probably running the agent on the terminal's alternate screen. Rows that leave the alternate screen do not enter Shepherd's host scrollback, so a larger line count cannot recover them.
...
- Never run `shepherd server stop` from an active session unless the user explicitly intends to stop the server and its pane processes.
- Never kill the main Shepherd process. Use named test sessions for experiments that need an isolated server.
