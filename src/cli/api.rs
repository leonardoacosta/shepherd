const API_SCHEMA_JSON: &str = include_str!("../../docs/next/api/shepherd-api.schema.json");
...
fn api_snapshot(args: &[String]) -> std::io::Result<i32> {
    if !args.is_empty() {
        eprintln!("usage: shepherd api snapshot");
        return Ok(2);
    }
...
fn schema_summary_text() -> std::io::Result<String> {
    ...
    Ok(format!(
        "Shepherd API schema\nprotocol: {}\nschema_version: {}\nschemas: {}\n\nUse `shepherd api schema --json` to print the full schema.\nUse `shepherd api schema --output PATH` to write it to a file.\n",
        protocol,
        schema_version,
        schemas.join(", ")
    ))
...
fn print_api_help() {
    eprintln!("shepherd api commands:");
    eprintln!("  shepherd api snapshot");
    eprintln!("  shepherd api schema [--json | --output PATH]");
}

fn print_api_schema_help() {
    eprintln!("usage: shepherd api schema [--json | --output PATH]");
}
...
mod tests {
    ...
    fn schema_summary_text_stays_human_sized() {
        ...
        assert!(text.contains("Shepherd API schema"));
        assert!(text.contains("Use `shepherd api schema --json`"));
        assert!(text.len() < 400);
    }
}
