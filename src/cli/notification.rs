...
use crate::config::ToastShepherdPosition;
...
fn notification_show(args: &[String]) -> std::io::Result<i32> {
    let params = match parse_notification_show_args(args) {
        ...
        Err(NotificationShowArgError::Usage) => {
            eprintln!(
                "usage: shepherd notification show <title> [--body TEXT] [--position top-left|top-right|bottom-left|bottom-right] [--sound none|done|request]"
            );
            return Ok(2);
        }
...
fn parse_toast_position(value: &str) -> Result<ToastShepherdPosition, NotificationShowArgError> {
    match value {
        "top-left" => Ok(ToastShepherdPosition::TopLeft),
        "top-right" => Ok(ToastShepherdPosition::TopRight),
        "bottom-left" => Ok(ToastShepherdPosition::BottomLeft),
        "bottom-right" => Ok(ToastShepherdPosition::BottomRight),
...
fn print_notification_help() {
    eprintln!("shepherd notification commands:");
    eprintln!(
    ...
    fn notification_show_args_parse_title_body_and_position() {
        ...
        assert_eq!(
            ...
            NotificationShowParams {
                ...
                position: Some(ToastShepherdPosition::TopRight),
