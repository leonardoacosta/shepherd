...
fn pane_get(args: &[String]) -> std::io::Result<i32> {
    let Some(raw_pane_id) = args.first() else {
        eprintln!("usage: shepherd pane get <pane_id>");
        return Ok(2);
    };
    if args.len() != 1 {
        eprintln!("usage: shepherd pane get <pane_id>");
...
fn pane_current(args: &[String]) -> std::io::Result<i32> {
    let env_pane_id = std::env::var("SHEPHERD_PANE_ID")
        .ok()
...
fn parse_pane_neighbor_args(args: &[String]) -> Result<PaneNeighborParams, String> {
    ...
    let Some(direction) = direction else {
        return Err(
            "usage: shepherd pane neighbor --direction left|right|up|down [--pane ID|--current]"
                .into(),
        );
    };
...
fn parse_pane_focus_args(args: &[String]) -> Result<PaneFocusDirectionParams, String> {
    let params = parse_pane_neighbor_args(args).map_err(|_| {
        "usage: shepherd pane focus --direction left|right|up|down [--pane ID|--current]"
            .to_string()
    })?;
...
fn parse_pane_resize_args(args: &[String]) -> Result<PaneResizeParams, String> {
    ...
    let Some(direction) = direction else {
        return Err(
            "usage: shepherd pane resize --direction left|right|up|down [--amount FLOAT] [--pane ID|--current]"
                .into(),
        );
    };
...
fn pane_rename(args: &[String]) -> std::io::Result<i32> {
    let Some(raw_pane_id) = args.first() else {
        eprintln!("usage: shepherd pane rename <pane_id> <label>|--clear");
        return Ok(2);
    };
    if args.len() < 2 {
        eprintln!("usage: shepherd pane rename <pane_id> <label>|--clear");
...
fn pane_read(args: &[String]) -> std::io::Result<i32> {
    let Some(raw_pane_id) = args.first() else {
        eprintln!("usage: shepherd pane read <pane_id> [--source visible|recent|recent-unwrapped] [--lines N] [--format text|ansi] [--ansi]");
        return Ok(2);
    };
...
fn parse_pane_split_args(
    ...
    let Some(direction) = direction else {
        return Err(
            "usage: shepherd pane split [<pane_id>|--pane ID|--current] --direction right|down [--ratio FLOAT] [--cwd PATH] [--env KEY=VALUE] [--focus] [--no-focus]"
                .into(),
        );
    };
...
fn parse_pane_swap_args(args: &[String]) -> Result<PaneSwapParams, String> {
    ...
    match (directional, explicit) {
        ...
        _ => Err(
            "usage: shepherd pane swap --direction left|right|up|down [--pane ID|--current]\n       shepherd pane swap --source-pane ID --target-pane ID"
                .into(),
        ),
    }
}
...
fn pane_close(args: &[String]) -> std::io::Result<i32> {
    let Some(raw_pane_id) = args.first() else {
        eprintln!("usage: shepherd pane close <pane_id>");
        return Ok(2);
    };
    if args.len() != 1 {
        eprintln!("usage: shepherd pane close <pane_id>");
...
fn pane_send_text(args: &[String]) -> std::io::Result<i32> {
    if args.len() < 2 {
        eprintln!("usage: shepherd pane send-text <pane_id> <text>");
        return Ok(2);
    }
...
fn pane_send_keys(args: &[String]) -> std::io::Result<i32> {
    if args.len() < 2 {
        eprintln!("usage: shepherd pane send-keys <pane_id> <key> [key ...]");
        return Ok(2);
    }
...
fn pane_run(args: &[String]) -> std::io::Result<i32> {
    if args.len() < 2 {
        eprintln!("usage: shepherd pane run <pane_id> <command>");
        return Ok(2);
    }
...
fn pane_wait_output(args: &[String]) -> std::io::Result<i32> {
    let Some(raw_pane_id) = args.first() else {
        eprintln!("usage: shepherd pane wait-output <pane_id> (--match TEXT | --regex PATTERN) [--source visible|recent|recent-unwrapped] [--lines N] [--timeout MS] [--raw]");
        return Ok(2);
    };
    let pane_id = super::normalize_pane_id(raw_pane_id);
    let mut source = ReadSource::Recent;
...
fn pane_report_agent(args: &[String]) -> std::io::Result<i32> {
    let Some(raw_pane_id) = args.first() else {
        eprintln!("usage: shepherd pane report-agent <pane_id> --source ID --agent LABEL --state idle|working|blocked|unknown [--message TEXT] [--seq N] [--agent-session-id ID] [--agent-session-path PATH]");
        return Ok(2);
    };
...
fn pane_report_agent_session(args: &[String]) -> std::io::Result<i32> {
    let Some(raw_pane_id) = args.first() else {
        eprintln!("usage: shepherd pane report-agent-session <pane_id> --source ID --agent LABEL [--seq N] [--agent-session-id ID] [--agent-session-path PATH] [--session-start-source SOURCE]");
        return Ok(2);
    };
...
fn pane_release_agent(args: &[String]) -> std::io::Result<i32> {
    let Some(raw_pane_id) = args.first() else {
        eprintln!(
            "usage: shepherd pane release-agent <pane_id> --source ID --agent LABEL [--seq N]"
        );
        return Ok(2);
    };
...
fn pane_report_metadata(args: &[String]) -> std::io::Result<i32> {
    let Some(raw_pane_id) = args.first() else {
        eprintln!("usage: shepherd pane report-metadata <pane_id> --source ID [--agent LABEL] [--applies-to-source ID] [--title TEXT|--clear-title] [--display-agent TEXT|--clear-display-agent] [--state-label STATUS=TEXT] [--clear-state-labels] [--token NAME=VALUE] [--clear-token NAME] [--seq N] [--ttl-ms N]");
        return Ok(2);
    };
...
fn print_pane_help() {
    eprintln!("shepherd pane commands:");
    eprintln!("  shepherd pane list [--workspace <workspace_id>]");
    eprintln!("  shepherd pane current [--pane ID|--current]");
    eprintln!("  shepherd pane get <pane_id>");
    eprintln!("  shepherd pane layout [--pane ID|--current]");
    eprintln!("  shepherd pane process-info [--pane ID|--current]");
    eprintln!("  shepherd pane neighbor --direction left|right|up|down [--pane ID|--current]");
    eprintln!("  shepherd pane edges [--pane ID|--current]");
    eprintln!("  shepherd pane focus --direction left|right|up|down [--pane ID|--current]");
    eprintln!(
        "  shepherd pane resize --direction left|right|up|down [--amount FLOAT] [--pane ID|--current]"
    );
    eprintln!("  shepherd pane zoom [<pane_id>|--pane ID|--current] [--toggle|--on|--off]");
    eprintln!("  shepherd pane rename <pane_id> <label>|--clear");
    eprintln!("  shepherd pane read <pane_id> [--source visible|recent|recent-unwrapped] [--lines N] [--format text|ansi] [--ansi]");
    eprintln!(
        "  shepherd pane split [<pane_id>|--pane ID|--current] --direction right|down [--ratio FLOAT] [--cwd PATH] [--env KEY=VALUE] [--focus] [--no-focus]"
    );
    eprintln!("  shepherd pane swap --direction left|right|up|down [--pane ID|--current]");
    eprintln!("  shepherd pane swap --source-pane ID --target-pane ID");
    eprintln!("  shepherd pane move <pane_id> --tab <tab_id> --split right|down [--target-pane ID] [--ratio FLOAT] [--focus|--no-focus]");
    eprintln!("  shepherd pane move <pane_id> --new-tab [--workspace ID] [--label TEXT] [--focus|--no-focus]");
    eprintln!("  shepherd pane move <pane_id> --new-workspace [--label TEXT] [--tab-label TEXT] [--focus|--no-focus]");
    eprintln!("  shepherd pane close <pane_id>");
    eprintln!("  shepherd pane send-text <pane_id> <text>");
    eprintln!("  shepherd pane send-keys <pane_id> <key> [key ...]");
    eprintln!("  shepherd pane wait-output <pane_id> (--match TEXT | --regex PATTERN) [--source visible|recent|recent-unwrapped] [--lines N] [--timeout MS] [--raw]");
    eprintln!("  shepherd pane report-agent <pane_id> --source ID --agent LABEL --state idle|working|blocked|unknown [--message TEXT] [--seq N] [--agent-session-id ID] [--agent-session-path PATH]");
    eprintln!("  shepherd pane report-agent-session <pane_id> --source ID --agent LABEL [--seq N] [--agent-session-id ID] [--agent-session-path PATH]");
    eprintln!("  shepherd pane release-agent <pane_id> --source ID --agent LABEL [--seq N]");
    eprintln!("  shepherd pane report-metadata <pane_id> --source ID [--agent LABEL] [--applies-to-source ID] [--title TEXT|--clear-title] [--display-agent TEXT|--clear-display-agent] [--state-label STATUS=TEXT] [--clear-state-labels] [--token NAME=VALUE] [--clear-token NAME] [--seq N] [--ttl-ms N]");
    eprintln!("  shepherd pane run <pane_id> <command>");
}
...
mod tests {
    ...
    fn parse_pane_swap_args_rejects_mixed_forms() {
        ...
        assert!(err.contains("usage: shepherd pane swap"));
    }
    ...
    fn parse_pane_move_args_rejects_target_pane_without_tab() {
        ...
        assert!(err.contains("usage: shepherd pane move"));
    }
