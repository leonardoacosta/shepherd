...
impl App {
    ...
    pub(crate) fn refresh_new_shepherd_toast_context_for_update(
        &mut self,
...
mod tests {
    ...
    #[cfg(unix)]
    #[tokio::test]
    async fn shepherd_toast_context_uses_live_root_runtime_cwd_label() {
        let (_api_tx, api_rx) = tokio::sync::mpsc::unbounded_channel();
        let mut app = App::new(
            &crate::config::Config::default(),
            true,
        ...
        let stale_cwd = temp_root.join("__shepherd_original__");
        let live_cwd = temp_root.join("__shepherd_projects__");
    ...
    #[cfg(unix)]
    #[tokio::test]
    async fn delayed_shepherd_toast_context_uses_live_root_runtime_cwd_label() {
        let (_api_tx, api_rx) = tokio::sync::mpsc::unbounded_channel();
        let mut app = App::new(
            &crate::config::Config::default(),
            true,
        ...
        let terminal_id = workspace.terminal_id(root).cloned().unwrap();
        let temp_root = std::env::temp_dir().join(format!(
            "shepherd-delayed-toast-context-{}-{}",
            std::process::id(),
            std::time::SystemTime::now()
                .duration_since(std::time::UNIX_EPOCH)
                .unwrap()
    ...
    fn terminal_delivery_does_not_refresh_existing_targeted_toast() {
        ...
        let mut workspace = crate::workspace::Workspace::test_new("stale");
        workspace.custom_name = None;
        workspace.identity_cwd = "/__shepherd_original__".into();
        let root = workspace.tabs[0].root_pane;
        let terminal_id = workspace.terminal_id(root).cloned().unwrap();
        let workspace_id = workspace.id.clone();
        app.state.workspaces = vec![workspace];
        app.state.ensure_test_terminals();
        app.state.terminals.get_mut(&terminal_id).unwrap().cwd = "/__shepherd_projects__".into();
        app.state.active = None;
        app.state.selected = 0;
        app.state.mode = Mode::Terminal;
        ...
        app.state.toast = Some(crate::app::state::ToastNotification {
            ...
            context: "__shepherd_original__ · 1".into(),
            position: None,
