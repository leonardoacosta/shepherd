...
# SHEPHERD_INTEGRATION_ID=devin
# SHEPHERD_INTEGRATION_VERSION=3

set -eu

action="${1:-}"
hook_input_file="$(mktemp "${TMPDIR:-/tmp}/shepherd-devin-hook.XXXXXX")" || exit 0
...
SHEPHERD_HOOK_INPUT_FILE="$hook_input_file" python3 - <<'PY'
...
def load_session_list(project_dir: str | None):
    injected = os.environ.get("SHEPHERD_DEVIN_LIST_JSON")
...
hook_input = load_hook_input(os.environ.get("SHEPHERD_HOOK_INPUT_FILE"))
