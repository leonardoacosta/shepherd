...
if ([string]::IsNullOrWhiteSpace($env:SHEPHERD_SOCKET_PATH)) { exit 0 }
...
& shepherd pane report-agent-session $env:SHEPHERD_PANE_ID --source shepherd:copilot --agent copilot --agent-session-id $sessionId --seq $seq 2>$null | Out-Null
