...
# SHEPHERD_INTEGRATION_VERSION=8

set -eu

action="${1:-}"
hook_input_file="$(mktemp "${TMPDIR:-/tmp}/shepherd-claude-hook.XXXXXX")" || exit 0
...
SHEPHERD_ACTION="$action" SHEPHERD_HOOK_INPUT_FILE="$hook_input_file" python3 - <<'PY'
import json
import os
import random
import socket
...
action = os.environ.get("SHEPHERD_ACTION", "")
pane_id = os.environ.get("SHEPHERD_PANE_ID")
socket_path = os.environ.get("SHEPHERD_SOCKET_PATH")
hook_input_file = os.environ.get("SHEPHERD_HOOK_INPUT_FILE")
...
if hook_event_name == "SubagentStop":
    # SubagentStop is a completion event. Older Shepherd integrations mapped it
