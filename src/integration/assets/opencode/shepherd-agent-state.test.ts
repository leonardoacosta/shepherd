...
beforeEach(() => {
  ...
  process.env.SHEPHERD_SOCKET_PATH = "test.sock";
  process.env.SHEPHERD_PANE_ID = "test:p1";
});

async function loadPlugin() {
  importCounter += 1;
  const { ShepherdAgentStatePlugin } = await import(`./shepherd-agent-state.js?test=${importCounter}`);
  return ShepherdAgentStatePlugin();
}
