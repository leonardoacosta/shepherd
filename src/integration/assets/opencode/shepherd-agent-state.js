...
// SHEPHERD_INTEGRATION_VERSION=10
...
export const ShepherdAgentStatePlugin = async () => {
  if (
    process.env.SHEPHERD_ENV !== "1" ||
    !process.env.SHEPHERD_SOCKET_PATH ||
    !process.env.SHEPHERD_PANE_ID
  ) {
    return {};
  }
    ...
    event: async ({ event }) => {
      ...
      switch (type) {
        case "session.created":
          ...
          // creates are dropped above). Signal it so shepherd replaces the pane's
