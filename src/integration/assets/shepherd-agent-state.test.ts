...
const originalEnvironment = {
  SHEPHERD_ENV: process.env.SHEPHERD_ENV,
  SHEPHERD_OMP_IDLE_DEBOUNCE_MS: process.env.SHEPHERD_OMP_IDLE_DEBOUNCE_MS,
  SHEPHERD_PANE_ID: process.env.SHEPHERD_PANE_ID,
  SHEPHERD_SOCKET_PATH: process.env.SHEPHERD_SOCKET_PATH,
};
...
const integrations = [
  { name: "Pi", modulePath: "./pi/shepherd-agent-state.ts" },
  { name: "Oh My Pi", modulePath: "./omp/shepherd-agent-state.ts" },
] as const;

const socketPlugins = [
  {
    name: "OpenCode",
    modulePath: "./opencode/shepherd-agent-state.js",
    sessionID: "opencode-session",
  },
  { name: "Kilo", modulePath: "./kilo/shepherd-agent-state.js", sessionID: "kilo-session" },
] as const;
...
function configureIntegrationEnvironment(recordingSocketPath: string) {
  ...
  process.env.SHEPHERD_SOCKET_PATH = recordingSocketPath;
  process.env.SHEPHERD_PANE_ID = "test:p1";
}
...
async function startRecordingServer(name: string): Promise<unknown[]> {
  const recordingSocketPath = join(tmpdir(), `shepherd-${name}-${process.pid}.sock`);
...
for (const socketPlugin of socketPlugins) {
  test(`${socketPlugin.name} maps the Windows socket marker path to a named pipe endpoint`, async () => {
    const markerPath = `shepherd-${socketPlugin.name.toLowerCase()}-${process.pid}.sock`;
    configureIntegrationEnvironment(markerPath);
    Object.defineProperty(process, "platform", { value: "win32" });
    const connectedEndpoint = captureConnectionEndpoint();

    const { ShepherdAgentStatePlugin } = await importFresh(socketPlugin.modulePath);
    const plugin = await ShepherdAgentStatePlugin();
    await plugin.event({
      event: {
...
test("OpenCode stays disabled without the Shepherd socket environment", async () => {
  process.env.SHEPHERD_ENV = "1";
  process.env.SHEPHERD_PANE_ID = "test:p1";
  delete process.env.SHEPHERD_SOCKET_PATH;

  const { ShepherdAgentStatePlugin } = await importFresh("./opencode/shepherd-agent-state.js");

  expect(await ShepherdAgentStatePlugin()).toEqual({});
});

for (const integration of integrations) {
  test(`${integration.name} maps the Windows socket marker path to a named pipe endpoint`, async () => {
    const markerPath = `shepherd-${integration.name.toLowerCase().replaceAll(" ", "-")}-${process.pid}.sock`;
    configureIntegrationEnvironment(markerPath);
    Object.defineProperty(process, "platform", { value: "win32" });
    const connectedEndpoint = captureConnectionEndpoint();
...
test("Pi settlement preserves explicit blocked-state precedence", async () => {
  ...
  handlers.get("agent_start")?.({}, context);
  await waitFor(() => requestStates(requests).length === 2);
  eventHandlers.get("shepherd:blocked")?.({ active: true, label: "approval" }, context);
  await waitFor(() => requestStates(requests).length === 3);
...
test("Pi waits for a replacement session report before publishing state", async () => {
  const recordingSocketPath = join(tmpdir(), `shepherd-pi-session-order-${process.pid}.sock`);
  socketPath = recordingSocketPath;
  await rm(recordingSocketPath, { force: true });
...
test("Oh My Pi retries working before a queued idle state", async () => {
  ...
  process.env.SHEPHERD_OMP_IDLE_DEBOUNCE_MS = "0";
  const { handlers, pi } = createExtensionHarness();

  const { default: install } = await importFresh("./omp/shepherd-agent-state.ts");
  install(pi);
