...
fn enforce_agent_version_warns_when_binary_missing() {
    let requirement = AgentVersionRequirement {
        label: "kimi code",
        binary: "shepherd-test-binary-that-does-not-exist",
        args: &["--version"],
        min_version: "0.14.0",
    };
...
fn unique_base() -> PathBuf {
    ...
    std::env::temp_dir().join(format!(
        "shepherd-integration-install-test-{}-{}",
        std::process::id(),
...
#[test]
fn install_omp_preserves_non_shepherd_file_with_pi_install_name() {
    let _lock = integration_env_lock();
    let base = unique_base();
    let home = base.join("home");
    let ext_dir = home.join(".omp/agent/extensions");
...
fn outdated_integrations_treat_missing_version_marker_as_legacy() {
    ...
    fs::write(&extension_path, "// installed by shepherd\n").unwrap();
    std::env::set_var("HOME", &home);
...
fn outdated_integrations_detect_previous_pi_version() {
    ...
    fs::write(
        &extension_path,
        "// SHEPHERD_INTEGRATION_ID=pi\n// SHEPHERD_INTEGRATION_VERSION=4\n",
    )
    .unwrap();
    std::env::set_var("HOME", &home);
...
fn claude_v1_integration_status_is_outdated() {
    ...
    fs::write(
        &hook_path,
        "#!/bin/sh\n# SHEPHERD_INTEGRATION_ID=claude\n# SHEPHERD_INTEGRATION_VERSION=1\n",
    )
    .unwrap();
    std::env::set_var("HOME", &home);
...
#[test]
fn uninstall_claude_removes_shepherd_hooks_and_preserves_others() {
    let _lock = integration_env_lock();
    let base = unique_base();
    let home = base.join("home");
    let claude_dir = home.join(".claude");
...
#[test]
fn uninstall_codex_removes_shepherd_hooks_and_leaves_config_alone() {
    let _lock = integration_env_lock();
    let base = unique_base();
    let home = base.join("home");
    let codex_dir = home.join(".codex");
    ...
    for event in COPILOT_REMOVED_LIFECYCLE_HOOK_EVENTS {
        if let Some(entries) = settings["hooks"].get(event) {
            assert!(
                ...
                "expected shepherd hooks.{event} entries to be removed"
            );
        }
    }
    assert!(settings["hooks"].get("sessionStart").is_none());
...
fn copilot_v1_integration_status_is_outdated() {
    ...
    fs::write(
        &hook_path,
        "#!/bin/sh\n# SHEPHERD_INTEGRATION_ID=copilot\n# SHEPHERD_INTEGRATION_VERSION=1\n",
    )
    .unwrap();
    std::env::set_var("HOME", &home);
...
#[test]
fn uninstall_copilot_removes_shepherd_hooks_and_preserves_others() {
    let _lock = integration_env_lock();
    let base = unique_base();
    let home = base.join("home");
    let copilot_dir = home.join(".copilot");
...
#[test]
fn uninstall_devin_removes_shepherd_hooks_and_preserves_others() {
    let _lock = integration_env_lock();
    let base = unique_base();
    let xdg_config = base.join("xdg");
    let devin_dir = xdg_config.join("devin");
...
#[test]
fn uninstall_droid_removes_shepherd_hooks_and_preserves_others() {
    let _lock = integration_env_lock();
    let base = unique_base();
    let home = base.join("home");
    let droid_dir = home.join(".factory");
...
fn install_hermes_is_idempotent_for_enabled_entry() {
    ...
    assert_eq!(config.matches("shepherd-agent-state").count(), 1);
...
fn install_hermes_is_idempotent_for_quoted_flat_plugin_entry() {
    ...
    fs::write(
        hermes_dir.join("config.yaml"),
        "plugins:\n  - \"shepherd-agent-state\" # installed by shepherd\n",
    )
    .unwrap();
    std::env::set_var("HOME", &home);
...
fn uninstall_hermes_removes_plugin_and_enabled_entry() {
    ...
    assert!(!config.contains("shepherd-agent-state"));
...
fn uninstall_hermes_removes_flow_plugin_list_entry() {
    ...
    fs::write(
        hermes_dir.join("config.yaml"),
        "plugins: [other-plugin, shepherd-agent-state]\n",
    )
    .unwrap();
    std::env::set_var("HOME", &home);
...
fn bundled_integration_assets_report_session_refs() {
    ...
    assert!(
        CODEX_HOOK_ASSET.contains("SHEPHERD_HOOK_INPUT_FILE")
    ...
    assert!(KIMI_HOOK_ASSET.contains("source\": \"shepherd:kimi"));
    ...
    assert!(DEVIN_HOOK_ASSET.contains("SHEPHERD_DEVIN_LIST_JSON"));
    ...
    assert!(KILO_PLUGIN_ASSET.contains("SOURCE = \"shepherd:kilo\""));
    ...
    assert!(QODERCLI_HOOK_ASSET.contains("SHEPHERD_PANE_ID"));
    ...
    assert!(CURSOR_HOOK_ASSET.contains("SHEPHERD_INTEGRATION_ID=cursor"));
    ...
    assert!(MASTRACODE_HOOK_ASSET.contains("SHEPHERD_INTEGRATION_ID=mastracode"));
    assert!(MASTRACODE_HOOK_ASSET.contains("SHEPHERD_INTEGRATION_VERSION=2"));
    ...
    assert!(GROK_HOOK_ASSET.contains("SHEPHERD_INTEGRATION_ID=grok"));
    ...
    assert!(GROK_HOOK_ASSET.contains("shepherd:grok"));
...
#[test]
fn uninstall_qodercli_removes_shepherd_hooks_and_preserves_others() {
    let _lock = integration_env_lock();
    let base = unique_base();
    let qoder_dir = base.join(".qoder");
    fs::create_dir_all(&qoder_dir).unwrap();
    ...
    assert!(session_start[0]
        ...
        .is_some_and(|command| {
            command.starts_with("bash ")
                && command.contains("shepherd-agent-state.sh")
...
#[test]
fn uninstall_cursor_removes_shepherd_hooks_and_preserves_others() {
    let _lock = integration_env_lock();
    let base = unique_base();
    let cursor_dir = base.join(".cursor");
    fs::create_dir_all(&cursor_dir).unwrap();
...
fn install_mastracode_writes_hook_and_updates_hooks_json() {
    ...
    for (event, action) in MASTRACODE_HOOK_EVENTS {
        let entries = hooks.get(event).and_then(Value::as_array).unwrap();
        assert_eq!(entries.len(), 1, "{event} should have one Shepherd hook");
...
fn install_grok_writes_hook_and_config() {
    ...
    assert!(command.contains("shepherd-agent-state.sh"));
...
#[test]
fn uninstall_mastracode_removes_shepherd_hooks_and_preserves_others() {
    let _lock = integration_env_lock();
    let base = unique_base();
    let original_home = std::env::var_os("HOME");
    std::env::set_var("HOME", &base);
...
fn grok_v1_integration_status_is_current() {
    ...
    fs::create_dir_all(&grok_dir).unwrap();
    std::env::set_var(GROK_CONFIG_DIR_ENV_VAR, &grok_dir);
    // A real install writes both the hook script and hooks/shepherd.json.
    install_grok().unwrap();
...
fn grok_status_reports_outdated_when_hook_config_missing_or_broken() {
    ...
    fs::write(
        &config_path,
        r#"{"hooks":{"SessionStart":[{"hooks":[{"type":"command","command":"echo shepherd-agent-state.sh"}]}]}}"#,
    )
    .unwrap();
    assert_eq!(grok_state(), IntegrationStatusKind::Outdated);
...
fn grok_dir_honors_grok_home_after_config_dir_seam() {
    ...
    // The shepherd-level test seam still wins over GROK_HOME when set.
