...
fn restore_workspace(
    ...
    (
        Some(Workspace {
            ...
            cached_session_status: crate::workspace::SessionStatusSnapshot::default(),
...
mod tests {
    ...
    fn restored_worktree_space_membership_drops_missing_checkout() {
        let missing =
            std::env::temp_dir().join(format!("shepherd-missing-worktree-{}", std::process::id()));
    ...
    fn restore_rehydrates_agent_session_metadata() {
        ...
        assert_eq!(preserved.source, "shepherd:hermes");
    ...
    async fn restore_carries_persisted_agent_session_metadata() {
        ...
        assert_eq!(session.source, "shepherd:opencode");
