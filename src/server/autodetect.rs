//! Auto-detect launch behavior for the `shepherd` command.
//!
//! When the user runs `shepherd` with no subcommand:
...
/// directory where the user ran `shepherd`.
pub(crate) const STARTUP_CWD_ENV_VAR: &str = "SHEPHERD_STARTUP_CWD";

// ---------------------------------------------------------------------------
// Server detection
// ---------------------------------------------------------------------------

/// Checks whether a shepherd server is currently listening on the client socket.
///
...
/// Checks whether a shepherd server is listening at a specific socket path.
...
fn validate_running_server_compatibility() -> io::Result<()> {
    let Some(status) = read_server_status()? else {
        return Err(io::Error::other(format!(
            "a shepherd server is listening, but its status API is unavailable.\n\n{}\nIf that fails, stop the old server process manually.",
    ...
    Err(io::Error::other(format!(
        "Shepherd was updated, but this session is still running the old server.\n\nserver: v{} protocol {}\nclient: v{} protocol {}\n\n{}",
...
/// Spawns the shepherd server as a background daemon process.
///
...
/// - Inherits relevant environment variables (`XDG_CONFIG_HOME`, `SHEPHERD_SESSION`,
...
pub fn spawn_server_daemon() -> io::Result<u32> {
    ...
    let child = command.spawn().map_err(|err: io::Error| {
        io::Error::new(
            err.kind(),
            format!("failed to spawn shepherd server: {err}"),
        )
    })?;
...
pub fn wait_for_server_socket(socket_path: &Path, timeout: Duration) -> io::Result<()> {
    ...
    Err(io::Error::new(
        io::ErrorKind::TimedOut,
        format!(
            "server did not become ready within {}s (socket: {}). The background server may still be starting; try `shepherd` again, or check {}",
            timeout.as_secs(),
            socket_path.display(),
            crate::session::data_dir().join("shepherd-server.log").display()
        ),
    ))
}
...
///
/// This is the entry point called from `main.rs` when the user runs `shepherd`
...
mod tests {
    ...
    fn server_daemon_command_clears_socket_overrides_for_explicit_session() {
        ...
        std::env::set_var("SHEPHERD_CLIENT_SOCKET_PATH", "/tmp/inherited-client.sock");
        ...
        let command = build_server_daemon_command(PathBuf::from("/tmp/shepherd-test"));
        ...
        assert!(envs.iter().any(|(key, value)| {
            *key == OsStr::new("SHEPHERD_CLIENT_SOCKET_PATH") && value.is_none()
        }));
        std::env::remove_var(crate::api::SOCKET_PATH_ENV_VAR);
        std::env::remove_var("SHEPHERD_CLIENT_SOCKET_PATH");
        std::env::remove_var(crate::session::SESSION_ENV_VAR);
        crate::session::clear_explicit_session_for_test();
    }
...
"#,
    ...
    fn validate_running_server_compatibility_names_session_commands_for_protocol_mismatch() {
        ...
        assert!(
            message.contains("Run `shepherd session stop work`"),
            "unexpected error: {message}"
        );
        assert!(
            message.contains("then run `shepherd session attach work` again"),
            "unexpected error: {message}"
        );
