...
impl HeadlessServer {
    ...
    pub(super) fn perform_live_handoff(
        ...
        let token = crate::server::handoff::generate_token()?;
        ...
        if pane_by_terminal.len() > crate::server::handoff::MAX_FDS_PER_HANDOFF {
            ...
            return Err(io::Error::new(
                ...
                format!(
                    "live handoff supports at most {} panes in one update; close panes or restart shepherd normally",
        ...
        if let Err(err) = crate::server::handoff::report_committed(&mut stream) {
            ...
            return Err(io::Error::other(format!(
                "handoff replacement server was ready, but commit failed: {err}"
            )));
        }
