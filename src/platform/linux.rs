...
mod tests {
    ...
    fn read_clipboard_image_rejects_xclip_text_served_for_image_target() {
        let _guard = env_lock().lock().unwrap();
        let temp_dir =
            std::env::temp_dir().join(format!("shepherd-fake-xclip-{}", std::process::id()));
    ...
    fn desktop_notification_separates_option_like_titles() {
        ...
        let path =
            std::env::temp_dir().join(format!("shepherd-notify-send-args-{}", std::process::id()));
