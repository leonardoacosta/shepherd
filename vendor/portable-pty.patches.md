...
equivalent fix or exposes an option that lets Shepherd keep the same behavior.
...
search path. Shepherd must never load another application's DLL from `PATH`. The
Windows package ships a pinned Microsoft ConPTY runtime under `conpty/`; Shepherd
...
`SHEPHERD_WINDOWS_CONPTY=system` to bypass the bundle during compatibility
recovery.

remove when: upstream `portable-pty` exposes hash-verified app-local and system
ConPTY selection with constrained DLL loading and no bare DLL search, or Shepherd
...
upstream issue: https://github.com/ogulcancelik/herdr/issues/1041
...
reason: Shepherd needs to launch `cmd.exe /d /c` with the user-authored command
...
support or Shepherd replaces this launch path.
