...
$root = Join-Path $env:RUNNER_TEMP ("shepherd-installer-test-" + [Guid]::NewGuid().ToString("N"))
$webRoot = Join-Path $root "web"
$shepherdHome = Join-Path $root "home"
$installDir = Join-Path $root "bin"
New-Item -ItemType Directory -Force -Path $webRoot | Out-Null
Copy-Item -LiteralPath $archive -Destination (Join-Path $webRoot "shepherd-windows-x86_64.zip")
...
$manifest = @{
    ...
    assets = @{
        "windows-x86_64" = @{
            url = "http://127.0.0.1:$port/shepherd-windows-x86_64.zip"
            sha256 = $hash
            format = "zip"
        }
    }
...
try {
    ...
    $releaseDir = Get-ChildItem -LiteralPath (Join-Path $shepherdHome "packages\standalone\releases") -Directory |
    ...
    if (-not (Test-Path -LiteralPath (Join-Path $releaseDir.FullName "shepherd.exe") -PathType Leaf)) {
    ...
    $env:SHEPHERD_HOME = $oldShepherdHome
