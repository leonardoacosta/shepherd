...
const SOURCE_CHECKOUT = "/home/can/Projects/shepherd";
const DEFAULT_BASE = "master";
const EXTENSION_DIR = dirname(fileURLToPath(import.meta.url));
const EXTENSION_NAME = "shepherd-worktree";

type ShepherdWorktreeResult = {
  result?: {
...
export default function (pi: ExtensionAPI) {
  pi.registerTool({
    name: "shepherd_start_worktree",
    label: "Start Shepherd Worktree",
    description:
      "Create a Shepherd-linked git worktree from the Shepherd master checkout, continue the active pi session in it, " +
      "start pi in the new Shepherd pane, then shut down and clean up the old pane.",
    promptSnippet: "Create a Shepherd worktree workspace and continue the active pi session in it",
    promptGuidelines: [
      "Use shepherd_start_worktree when work in the Shepherd repo should continue in a fresh git worktree.",
      "shepherd_start_worktree creates the checkout from /home/can/Projects/shepherd on master by default.",
      "Prefer passing a clear branch name such as issue/123-short-slug when the work relates to an issue.",
      "After shepherd_start_worktree succeeds, the current pi process will shut down and the old Shepherd pane will close.",
    ],
    parameters: Type.Object({
      branch: Type.Optional(
        Type.String({
          description:
            "Branch name for the new worktree. If omitted, Shepherd generates a worktree/* branch.",
        }),
      ),
      base: Type.Optional(
        Type.String({
      ...
      label: Type.Optional(
        Type.String({
          description: "Workspace label for the new Shepherd worktree workspace.",
        }),
      ),
      closeOldPane: Type.Optional(
        Type.Boolean({
          description: "Close the old Shepherd pane after the old pi process exits. Defaults to true.",
        }),
      ),
    ...
    async execute(_toolCallId, params, signal, _onUpdate, ctx) {
      return startShepherdWorktree(pi, ctx, signal, {
  ...
  pi.registerCommand("shepherd-worktree-start", {
    description:
      "Create a Shepherd worktree from master, continue this pi session in it, and clean up the old pane",
    handler: async (args, ctx) => {
      await ctx.waitForIdle();
      try {
        const parsed = parseCommandArgs(args ?? "");
        const result = await startShepherdWorktree(pi, ctx, undefined, parsed);
...
async function startShepherdWorktree(
  pi: ExtensionAPI,
  ctx: ExtensionContext,
  signal: AbortSignal | undefined,
  options: StartOptions,
) {
  if (process.env.SHEPHERD_ENV !== "1") {
    throw new Error("shepherd_start_worktree must run inside a Shepherd-managed pane");
  }

  const oldPaneId = process.env.SHEPHERD_PANE_ID;
  if (options.closeOldPane && !oldPaneId) {
    throw new Error("SHEPHERD_PANE_ID is missing; cannot close the old Shepherd pane safely");
  }
  ...
  ctx.ui.setStatus("shepherd-worktree", "creating worktree");

  let newSessionFile: string | undefined;
  try {
    const created = await createShepherdWorktree(pi, signal, sourceCheckout, options);
    ...
    ctx.ui.setStatus("shepherd-worktree", undefined);
    ctx.ui.notify(`Started pi in Shepherd worktree: ${worktreePath}`, "info");
    ctx.shutdown();
      ...
      content: [
        {
          type: "text" as const,
          text:
            `Started replacement pi in Shepherd worktree: ${worktreePath}\n` +
            `Workspace: ${created.workspaceId ?? "unknown"}\n` +
            `Pane: ${created.rootPaneId}\n` +
            `Branch: ${created.branch ?? "generated by Shepherd"}\n\n` +
...
async function createShepherdWorktree(
  pi: ExtensionAPI,
  signal: AbortSignal | undefined,
  sourceCheckout: string,
  options: StartOptions,
  ...
  const json = await shepherdJson(pi, args, signal, sourceCheckout, 120_000);
  const worktreePath = json.result?.worktree?.path;
  const rootPaneId = json.result?.root_pane?.pane_id;
  if (!worktreePath || !rootPaneId) {
    throw new Error("Shepherd worktree create response did not include worktree.path and root_pane.pane_id");
  }
...
async function runInNewPane(
  ...
  await shepherd(pi, ["pane", "run", paneId, command], signal, undefined, 10_000);
}
...
async function scheduleOldPaneCleanup(
  ...
  const cleanup = [
    ...
    "rm -f -- \"$old_session\"",
    "shepherd pane close \"$old_pane\" >/dev/null 2>&1 || true",
  ].join("; ");
...
async function shepherdJson(
  pi: ExtensionAPI,
  args: string[],
  signal: AbortSignal | undefined,
  cwd: string | undefined,
  timeout: number,
): Promise<ShepherdWorktreeResult> {
  const result = await shepherd(pi, args, signal, cwd, timeout);
  const raw = result.stdout.trim() || result.stderr.trim();
  let json: ShepherdWorktreeResult;
  try {
    json = JSON.parse(raw) as ShepherdWorktreeResult;
  } catch {
    throw new Error(`Shepherd returned non-JSON output for ${args.join(" ")}: ${raw}`);
  }
  if (json.error) {
    throw new Error(`${json.error.code ?? "shepherd_error"}: ${json.error.message ?? "unknown Shepherd error"}`);
  }
  return json;
}
...
async function shepherd(
  ...
  const result = await pi.exec("shepherd", args, { cwd, signal, timeout });
  if (result.code !== 0) {
    throw new Error(`shepherd ${args.join(" ")} failed: ${result.stderr || result.stdout}`);
  }
  return result;
}
