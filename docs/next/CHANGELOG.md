...
- Added 12 more Agent tokens that Shepherd resolves itself, alongside `spend`: `savings` and `cache_read_tokens` from the same `llmtrim` source; `instruction_total_bytes`, `instruction_floor_warn`, `instruction_growth_pct`, and `instruction_dollars_per_1k_turns` from a separate `context-floor` provider; `model`, `context_tokens`, `context_window_tokens`, and `session_count` from Shepherd's own persisted session snapshot store; and `quota_remaining` and `reset_at`, read-only from Shepherd's own credential state directory. Each of the four sources fails and recovers independently — one source being unavailable elides only its own tokens, never another source's, and configuring no token from a source runs no adapter for it. The companion (`shepherd-state`) is not invoked for any of these facts; it remains required only for harness hook ingest (`SessionStart`, `UserPromptSubmit`, `PermissionRequest`, `Stop`, `Notification`, `SessionEnd`), which is unaffected.
- Added `context_occupancy_pct` and `context_handoff_warning` Agent tokens, a pure derivation over `context_tokens`/`context_window_tokens` (no source of their own, no separate adapter) reporting the session's context-window occupancy as a whole percent and flagging when it has crossed the handoff threshold.
...
- Pane and agent API responses now expose a typed `state_source` describing whether the current agent state came from screen detection or a reported source, and whether that source has exclusive lifecycle authority or mixed authority. `screen_detection_skipped` is retained and now derives from the same projection. Settings describes integrations as reporting state or providing session identity in a number of panes, or as installed and not currently observed, without implying health.
- Space sidebar rows accept two new tokens, `proposals` and `beads`, reporting the change proposals and tracked issues in the space's own checkout as `op: 2o 1ip` and `bd: 3o 1r 0b`. Shepherd computes them natively on its own refresh timer, slower than Git status, and only when a configured row uses them, so an unconfigured token spawns no process. Counts are resolved once per checkout, so linked worktrees report their own work queue and several spaces on one checkout share a single refresh. `openspec` and `bd` are optional: a provider that is missing, fails, times out, or returns an unexpected payload leaves only its own token without a value, which elides like any other absent token.
- Added a disabled-by-default `[ui.right_panel]` chrome panel that renders configured token rows on the right edge using the same Agent-sidebar token vocabulary, resolver, elision, and styling rules as the topbar. Both side panels span the full terminal height and are clamped so at least one main-content column survives; a panel that resolves no rows reserves nothing. A chrome panel hosts no terminal runtime and takes no part in focus routing, which is what distinguishes it from the plugin dock.
- Added a `server.config.edit` operation and a Settings action that open the resolved server `config.toml` in an editor pane, using `VISUAL` then `EDITOR` then `vi` on Unix and safe command-line parsing with a Notepad fallback on Windows. One editor runs per session and a duplicate request returns the existing pane. After the editor exits, valid changes reload, invalid or unreadable changes are left on disk with diagnostics while the last valid configuration keeps running, and a deliberate removal reloads defaults. Shepherd never deletes the configuration file. Completion is published as `server.config_edit_finished`.
- Added 8 more Agent tokens that Shepherd resolves itself: `transcript_cost`, `transcript_input_tokens`, `transcript_output_tokens`, `transcript_cache_read_tokens`, `transcript_cache_write_tokens`, `transcript_last_context_tokens`, `transcript_message_count`, and `transcript_duration`, read by Shepherd directly from a pane's own Claude Code transcript file. Unlike every other session-provider token, these resolve per-pane rather than per-workspace, since two panes in the same workspace can run different Claude Code sessions with different transcripts.

### Changed
- The sidebar is now one flat list: each space row is followed directly by the agent rows running inside that space, with no second section and no draggable divider. An agent row is the switch target, activating its space, tab, and pane from anywhere; tabs no longer get rows of their own. A space running no detected agent shows an explicit `no agents` row, so an empty space is distinguishable from a collapsed one. Worktree grouping and its chevron, space drag-reorder, shortcut numbering by visible position, scrolling, and the shared `ui.agent_panel_sort` control are unchanged and now act on the one list. The sidebar section split is removed: the draggable divider is gone and the position previously kept in the session file is ignored on restore, which restores without error.
- The sidebar now anchors the full-height left edge, and the desktop topbar renders beside it across the main content instead of above it. Existing `[ui.topbar] rows` values stay valid and no token changed; only the region the topbar renders into moves.
