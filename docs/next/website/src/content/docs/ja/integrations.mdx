...
Herdr はデフォルトで `~/.grok` を使い、`GROK_HOME` が設定されていればそちらを使います。Grok の設定ディレクトリはあらかじめ存在している必要があります。Grok はそのディレクトリ内の `hooks/*.json` をすべてマージするため、インストールは `hooks/herdr-agent-state.sh` の隣に Herdr の `SessionStart` エントリを持つ自己完結な `hooks/herdr.json` を書き込み、ほかのフックファイルは変更しません。アンインストールはその 2 つの Herdr 管理ファイルだけを削除します。
