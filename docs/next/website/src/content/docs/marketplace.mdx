---
title: Marketplace
description: Discover community Shepherd plugins on GitHub, and get your own plugin listed.
---

The Shepherd plugin marketplace is a discoverable index of community plugins.
Browse it at [shepherd.dev/plugins](/plugins/). It is an automatic index of public
GitHub repositories, not a reviewed catalog.

## Browse plugins

The [marketplace](/plugins/) lists every public repository tagged with the
GitHub topic `shepherd-plugin`. Search by name, owner, description, or language,
...
itself, not that Shepherd vetted it, so the
...
Publish a normal public GitHub repository with a `shepherd-plugin.toml` manifest at
...
Add the GitHub topic `shepherd-plugin` to a public repository. That topic is the
...
The index does not parse `shepherd-plugin.toml` yet, so manifest fields like the
plugin `id`, declared `platforms`, and `min_shepherd_version` are not shown in v1.
