...
description: Use Shepherd's layout, pane, and agent primitives to coordinate coding agents from scripts or other agents.
---

Shepherd can act as an automation layer for coding agents. A script can control them, or one agent can create work for other agents, inspect their state, and collect their results. The important part is choosing the primitive that matches the job.
...
```bash
created=$(shepherd workspace create --cwd ~/project --label api --no-focus)
pane_id=$(printf '%s\n' "$created" | jq -r '.result.root_pane.pane_id')

split=$(shepherd pane split "$pane_id" --direction right --no-focus)
...
Moving a pane to another workspace changes its workspace-qualified pane ID. After any `pane move`, continue with `.result.move_result.pane.pane_id`; the response keeps the old value at `.result.move_result.previous_pane_id`. A running process keeps its launch-time Shepherd environment, but the old `SHEPHERD_PANE_ID` remains an alias for that terminal, so `--current` stays safe. New commands can still resolve the agent by name after the move, but a wait already in progress ends with `agent_not_running`.

Use pane commands for shells, tests, servers, CI watchers, and other ordinary terminal processes. Use agent commands when Shepherd needs to understand which agent is running or whether it is `working`, `blocked`, `done`, `idle`, or `unknown`.
...
`agent start` returns only after Shepherd detects the expected agent in the same terminal and marks it ready for interactive input. Startup waits for 30 seconds by default; `--timeout` must be greater than 3000 and no more than 300000 milliseconds.

```bash
shepherd agent start reviewer --kind codex --pane "$review_pane" -- -m gpt-5.4
```
...
```bash
shepherd agent get w1:p2
shepherd agent rename w1:p2 reviewer
```
...
Full-screen agents such as Claude Code and OpenCode may render in the terminal's alternate screen. Alternate-screen rows do not enter Shepherd's host scrollback. `--lines` can only request more rows from the pane's available screen and host scrollback; it cannot create missing history. If increasing `--lines` returns no additional response text, the pane is probably using the alternate screen and those response rows are no longer retained. A larger font or smaller pane makes this more likely.
...
```bash
split=$(shepherd pane split --current --direction right --no-focus)
review_pane=$(printf '%s\n' "$split" | jq -r '.result.pane.pane_id')
shepherd agent start reviewer --kind codex --pane "$review_pane" -- -m gpt-5.4
shepherd agent prompt reviewer "Review the current diff" --wait --timeout 120000
...
shepherd agent read reviewer --source recent-unwrapped --lines 80
...
```bash
shepherd pane run w1:p3 "just test --watch"
shepherd pane wait-output w1:p3 --regex "passed|failed" --timeout 120000
```
