---
title: Socket API
description: Control a running Shepherd server from scripts, tools, and coding agents.
---

Shepherd exposes a local socket API for scripts and agents that need to inspect or control a running session.
...
| Layer | Use it for |
| --- | --- |
| Agent skill | Teaching a coding agent how to use Shepherd from inside a pane. |
...
The installed CLI can print the socket protocol schema bundled with that Shepherd
binary:
...
Plain `shepherd api schema` prints a short summary. `--json` prints the full JSON
...
```bash
shepherd workspace create --cwd ~/project --label api
```

Create a tab:

```bash
shepherd tab create --label logs
```

Split a pane and run a command:

```bash
shepherd pane split w1:p1 --direction right
shepherd pane run w1:p2 "npm test"
```

Inspect and rearrange panes:

```bash
shepherd pane layout --current
shepherd pane neighbor --direction right --current
shepherd pane resize --direction right --amount 0.1 --current
shepherd pane swap --direction right --current
shepherd pane zoom --on --current
shepherd pane split w1:p1 --direction right --ratio 0.333
```

Wait for an agent:

```bash
shepherd agent wait w1:p1 --until done
```

Read pane output:

```bash
shepherd pane read w1:p2 --source recent --lines 50
```
...
| Area | Methods |
| --- | --- |
| Server | `ping`, `server.stop`, `server.reload_config`, `server.live_handoff`, `server.agent_manifests`, `server.reload_agent_manifests`, `server.config.edit` |
...
| Pane | `pane.split`, `pane.swap`, `pane.move`, `pane.zoom`, `pane.layout`, `pane.process_info`, `pane.neighbor`, `pane.edges`, `pane.focus_direction`, `pane.resize`, `pane.list`, `pane.current`, `pane.focus`, `pane.get`, `pane.rename`, `pane.send_text`, `pane.send_keys`, `pane.send_input`, `pane.read`, `pane.graphics.info`, `pane.graphics.set`, `pane.graphics.clear`, `pane.graphics.stream`, `pane.report_agent`, `pane.report_agent_session`, `pane.report_metadata`, `pane.clear_agent_authority`, `pane.release_agent`, `pane.close`, `pane.wait_for_output` |
| Popup | `popup.close` |
...
From the CLI, `shepherd api snapshot` prints the live `session.snapshot` response
as JSON for clients and agents that want a simple bootstrap command.

`server.live_handoff` hands the current session over to a replacement Shepherd
server during self-update or binary replacement. It is an internal maintainer
surface for coordinated server swap, not a general-purpose session export.
Normal automation should prefer the CLI update flow instead of calling this
method directly.
...
`pane.send_keys` and `pane.send_input.keys` accept Shepherd key-combo strings:
...
{"id":"req_split","method":"pane.split","params":{"direction":"right","ratio":0.333,"env":{"SHEPHERD_ROLE":"tests"}}}
...
Shepherd returns that pane. When it is omitted, Shepherd returns the active focused
pane.
...
Shepherd replies with `ok`, send one JSON header and then exactly `data_length` raw
...
provided, Shepherd creates the replacement tab first and then closes the old tab.
...
Process-launching methods accept an `env` object. Shepherd applies those key/value
pairs to the newly launched process only. Shepherd also injects `SHEPHERD_SOCKET_PATH`,
`SHEPHERD_ENV=1`, `SHEPHERD_WORKSPACE_ID`, `SHEPHERD_TAB_ID`, and `SHEPHERD_PANE_ID` into
managed pane processes. Shepherd-managed variables are authoritative when they
...
`pane.moved`; Shepherd does not emit fake pane close/create events for the moved
terminal process.
...
```bash
shepherd notification show "build failed" --body "api workspace" --position top-left --sound request
```
...
`title` is required and must contain visible text after control characters and repeated whitespace are removed. `body` is optional. Shepherd collapses newlines, tabs, carriage returns, and repeated whitespace into spaces, then trims notification text to 80 characters for `title` and 240 characters for `body`. An empty sanitized `title` returns `invalid_params`. `position` is optional and applies only when `ui.toast.delivery = "shepherd"`; desktop positions are relative to the full Shepherd frame, and omitted positions use `ui.toast.shepherd.position`. Terminal, system, and off delivery ignore `position`. `sound` is optional and can be `none`, `done`, or `request`; it defaults to `none` and plays only when the notification is shown.
...
Possible reasons are `shown`, `disabled`, `rate_limited`, `no_foreground_client`, and `busy`. `disabled` means `ui.toast.delivery = "off"`. `busy` means an existing in-app toast was not replaced. Terminal and system delivery are best-effort through the current foreground attached Shepherd client.

Set or clear the foreground client's outer terminal window title:

```json
{"id":"req_title","method":"client.window_title.set","params":{"title":"shepherd api"}}
{"id":"req_title_clear","method":"client.window_title.clear","params":{}}
```

`client.window_title.clear` restores Shepherd's default title. The response is
`type: "client_window_title"` with `changed` and reason `set`, `cleared`, or
`no_foreground_client`.

Worktree methods manage Git checkouts as Shepherd workspaces. `worktree.create` creates a checkout and returns the new `workspace`, `tab`, `root_pane`, and `worktree` records. If the requested branch already exists locally, it checks out that branch; otherwise it creates the branch from the requested base or `HEAD`. `worktree.open` opens an existing checkout or returns the already-open workspace. `worktree.remove` runs `git worktree remove` against a linked child workspace and never deletes the branch.
...
Use at most one of `workspace_id` or `cwd` for `worktree.list`, `worktree.create`, and `worktree.open`; omit both to use the active workspace. Use exactly one of `path` or `branch` for `worktree.open`. Raw socket `cwd` and `path` values must be absolute; the CLI expands relative `--cwd` and `--path` values before sending requests. Workspace responses include optional `worktree` provenance when a workspace belongs to a Shepherd worktree group. Worktree commands can emit `workspace.updated` when an existing workspace gains or changes worktree provenance.

Worktree commands also emit lifecycle events. `worktree.create` emits `workspace.created`, `tab.created`, `pane.created`, and `worktree.created`. `worktree.open` emits `worktree.opened`, and it also emits workspace/tab/pane creation events when it opens a new Shepherd workspace. `worktree.remove` emits `worktree.removed`; if the linked workspace is still open, it also emits `workspace.closed`.
...
`source` identifies the owner. Plugins use `plugin:<SHEPHERD_PLUGIN_ID>`; Shepherd
...
durable behavior should save the query under `SHEPHERD_PLUGIN_STATE_DIR` and
...
is a package with a `shepherd-plugin.toml` manifest. The manifest declares
...
Installed and linked plugins persist across restarts. Shepherd writes a
`plugins.json` registry file alongside `session.json` on `plugin.link`,
`plugin.unlink`, `plugin.enable`, and `plugin.disable`. The
`shepherd plugin install` and `shepherd plugin link` CLIs also write the same registry
when Shepherd is not running, then startup loads it automatically. On startup,
Shepherd re-reads each
...
Event hook `on` values are validated against the known Shepherd event names at link time. An unrecognised name is not an error — the link still succeeds — but the returned plugin info includes a warning (e.g. `"unknown event 'worktree.craeted'"`). Check the `warnings` field in the `plugin.link` and `plugin.list` responses.
...
```json
{"id":"req_plugin_link","method":"plugin.link","params":{"path":"/managed/plugin/shepherd-plugin.toml","enabled":true,"source":{"kind":"github","owner":"ogulcancelik","repo":"shepherd-plugin-examples","subdir":"worktree-bootstrap","requested_ref":"main","resolved_commit":"abc123","managed_path":"/data/plugins/github/<managed-checkout>","installed_unix_ms":1780000000000}}}
```

The path can be a plugin directory containing `shepherd-plugin.toml` or a direct
...
`min_shepherd_version` is required. The server refuses to link a plugin when the
field is missing, invalid, or newer than the running Shepherd binary.
...
command, and returns the Shepherd-built invocation context plus the started command
...
Shepherd injects `SHEPHERD_SOCKET_PATH`, `SHEPHERD_BIN_PATH`, `SHEPHERD_ENV=1`,
`SHEPHERD_PLUGIN_ID`, `SHEPHERD_PLUGIN_ROOT`, `SHEPHERD_PLUGIN_CONFIG_DIR`,
`SHEPHERD_PLUGIN_STATE_DIR`, `SHEPHERD_PLUGIN_CONTEXT_JSON`, and available
`SHEPHERD_WORKSPACE_ID`, `SHEPHERD_TAB_ID`, and `SHEPHERD_PANE_ID` values. Action
commands also receive `SHEPHERD_PLUGIN_ACTION_ID`; event hooks receive
`SHEPHERD_PLUGIN_EVENT` and `SHEPHERD_PLUGIN_EVENT_JSON`; pane commands receive
...
Event hooks run for enabled installed plugins when Shepherd emits a matching event
name such as `worktree.created`.

There is no Shepherd-managed plugin storage API in v1. `SHEPHERD_PLUGIN_CONFIG_DIR`
and `SHEPHERD_PLUGIN_STATE_DIR` are path discovery only; plugins own their files,
schemas, migrations, and cleanup.

Open a managed terminal UI:

```json
{"id":"req_plugin_pane","method":"plugin.pane.open","params":{"plugin_id":"example.board","entrypoint":"board","placement":"zoomed","target_pane_id":"w1:p1","env":{"SHEPHERD_ROLE":"board"},"focus":true}}
```
...
`SHEPHERD_PANE_ID` to its process. Popup launch returns `ok`; `popup.close` closes
...
behave like normal Shepherd panes, and `plugin.pane.focus` and `plugin.pane.close`
continue to operate on those panes.

## Socket transport

Shepherd uses newline-delimited JSON over a local socket. On Unix, that socket is a
...
The default socket lives under your Shepherd config directory.

Named sessions have separate sockets:

```text
~/.config/shepherd/shepherd.sock
~/.config/shepherd/sessions/<name>/shepherd.sock
```

Resolution order:

1. explicit CLI `--session <name>`
2. `SHEPHERD_SOCKET_PATH`
3. `SHEPHERD_SESSION=<name>`
4. default session socket

Use `SHEPHERD_SOCKET_PATH` only for low-level overrides.

For plugins, prefer invoking `SHEPHERD_BIN_PATH` and the CLI wrappers when you need
...
`pane.get`, `pane.list`, `agent.get`, and `agent.list` expose a read-only `agent_session` object when Shepherd has a stored native session reference:
...
`pane.get`, `pane.list`, `agent.get`, and `agent.list` also expose a read-only `state_source` object whenever the pane has current agent evidence, produced by one shared server-side projection so `PaneInfo` and `AgentInfo` always agree for the same terminal:
...
`effective.kind` is `"screen"` or `"reported"`. `"screen"` means the effective state came from screen-manifest detection; `"reported"` means it came from a hook-reported `source`, with `authority` either `"exclusive_lifecycle"` (a live full-lifecycle integration) or `"mixed"` (a session-only/custom report that can yield to a newer visible screen blocker). `reporter` is a separate, optional field: it stays populated with the currently effective hook report even when a newer screen blocker overrides `effective` to `"screen"`, so a still-active mixed reporter is never hidden. `reporter` is absent once the report is cleared, its owning process exits, or it otherwise stops being effective — a persisted or reported session identity alone (`agent_session`) never populates it. `state_source` is omitted when the pane has neither screen-detected nor reported agent state. `AgentInfo.screen_detection_skipped` remains a derived compatibility field: it is `true` exactly when `state_source.effective` is `"reported"` with `"exclusive_lifecycle"` authority.

`pane.get`, `pane.list`, `agent.get`, and `agent.list` also expose `foreground_cwd` when Shepherd can resolve the cwd of the process currently controlling the pane PTY. The existing `cwd` field remains the pane/workspace cwd used for labels, follow-cwd behavior, and restored session state.
...
Use `pane.report_metadata` when a user hook wants to customize presentation without taking over lifecycle state from a Shepherd integration.
...
Presentation text is normalized before storage. Shepherd trims surrounding whitespace, removes control characters, and caps `title`, `display_agent`, each state label, and token values at 80 characters. Empty normalized token values clear that key.
...
`workspace_id`, and the current `scroll` metrics whenever Shepherd observes a
...
The `server.config_edit_finished` subscription reports when a `server.config.edit` editor pane exits. Its event carries `pane_id`, `workspace_id`, `outcome` (`unchanged`, `reloaded`, `invalid`, or `editor_failed`), and bounded `diagnostics` strings — subscribers get exactly one event per pane per terminal outcome, and can reopen the same server-owned file immediately to correct an `invalid` edit.
...
```bash
shepherd pane read w1:p1 --source visible --lines 80
shepherd pane read w1:p1 --source recent --lines 120
shepherd pane read w1:p1 --source recent-unwrapped --lines 120
shepherd pane read w1:p1 --source detection
```
...
```bash
shepherd agent wait w1:p1 --until done
shepherd agent wait w1:p1 --until blocked
```
...
```json
{
  ...
  "result": {
    ...
    "manifests": [
      {
        "agent": "cursor",
        "source": "/home/me/.config/shepherd/agent-detection/cursor.toml",
...
`server.config.edit` opens the server's own resolved configuration file (`config.toml`) in an editor pane. It takes no parameters — the server always resolves its own path and rejects any client-supplied one by construction. A missing parent directory is created; a missing config file is never synthesized before the editor opens it.
...
At most one Shepherd-owned config editor pane exists per server session. A duplicate `server.config.edit` request while one is already open returns the same `pane` with `already_open: true` instead of starting a second editor; the operation continues even if the requesting client disconnects. If the server has no surface that can host a pane, the request fails with `config_editor_unavailable`.

On Unix, the editor is resolved from `VISUAL`, then `EDITOR`, then `vi`; on Windows, from the same precedence falling back to Notepad, using existing safe command-line parsing. The config path is always passed as a single trailing argument, never interpolated into a shell string, and is never registered for temporary-file cleanup — exiting the editor never deletes the config file.

When the editor pane exits, Shepherd compares the final bytes against the pre-edit snapshot and publishes exactly one `server.config_edit_finished` event per pane per outcome (see [Event subscriptions](#event-subscriptions)): `unchanged` when nothing changed, `reloaded` when changed bytes parsed as valid TOML (including when the editor process itself exited nonzero but left valid bytes, or when a previously-existing file was removed — Shepherd reloads defaults and never re-creates it), `invalid` when changed bytes are invalid TOML or unreadable (the last valid runtime configuration stays active and the invalid bytes are left on disk untouched), and `editor_failed` when the editor process exited nonzero without changing the file.
...
The response contains the same explain object printed by `shepherd agent explain --json`, including the final state, manifest source and version, matched rule, evaluated rule evidence, skip-state reason, idle fallback reason, and `screen_detection_skip_reason` when a full lifecycle hook authority makes screen rules non-authoritative.

Clients need a running server that supports `agent.explain`; after upgrading Shepherd, restart or live-handoff the server before relying on this method.
...
Shepherd has a protocol version for client/server compatibility. Protocol changes are reviewed with release compatibility in mind.

Check the server protocol with `ping` or `shepherd status` before depending on new behavior. Handle unknown fields gracefully.
