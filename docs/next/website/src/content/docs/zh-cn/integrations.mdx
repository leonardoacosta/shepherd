...
Herdr 默认使用 `~/.grok`,设置了 `GROK_HOME` 时使用后者。Grok 配置目录必须已经存在。Grok 会合并该目录下的所有 `hooks/*.json`,所以安装会在 `hooks/herdr-agent-state.sh` 旁边写入一个自包含的 `hooks/herdr.json`,其中包含 Herdr 的 `SessionStart` 条目,并且不会改动其他钩子文件。卸载只移除这两个 Herdr 拥有的文件。
