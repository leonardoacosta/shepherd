...
- Refreshed 2026-08-04 at `0a4623fcc7109a00fa685282c93ced6274ab64ed`; `dev` tracked `origin/dev` with only the declared dirty baseline (`improvements.md` plus the two sibling follow-on proposal directories) present, `just test-one settings` (34 passed), `just test-one integration` (126 passed), `just test-one config_io` (1 passed), and `just test-one sidebar` (125 passed) were green pre-change, `PROTOCOL_VERSION` (19) remained ahead of the latest released tag `v0.7.5` (17) so no bump is required, and `IntegrationTarget::ALL` held the 15 targets this change's per-target work covers. No other in-progress OpenSpec change edits `src/ui/settings.rs`, `src/app/state.rs`, `src/app/input/settings.rs`, or the other declared touch paths concurrently; `expose-agent-state-source` and `open-advanced-config-editor` declare overlapping future touches by design (they consume this change's Settings foundation) but have zero completed tasks and are not being executed in parallel.
- Geometry characterization (added in 2.1, protected through 2.2): `settings_sections_render_in_declared_order_when_width_permits`, `settings_sections_fall_back_to_overflow_viewport_when_labels_do_not_fit`, `settings_selected_row_stays_visible_after_resize_between_40x20_64x20_80x24`, `settings_mouse_hit_targets_resolve_same_row_as_keyboard_at_each_supported_size`, `settings_scrollbar_thumb_reflects_visible_range_for_long_sections`.
- Identity/render-purity characterization (protected, no regression permitted): `compute_view`/`render` purity — `render_settings_overlay` must keep taking `&AppState` and never mutate it (existing `experiments_pane_history_uses_settings_checkmark_marker` and siblings already assert this indirectly by rendering from an immutable `&AppState`); workspace/pane identity around Settings interaction is exercised by opening Settings, resizing, and mutating integration/display rows against `AppState::test_with_adversarial_identity_state()` followed by `AppState::assert_invariants_for_test()`, named `settings_interaction_preserves_adversarial_identity_invariants`.
- Existing input/render behavior protected as-is (regression bar for 2.x-4.x): theme preview-on-navigate plus cancel-restore (`settings_cancel_restores_previewed_theme_from_other_sections`), Sound/Toast/Experiments enter-and-mouse-click parity, Display's existing typed-row-identity scroll/selection model (`display_rows`, `normalize_display_selection`, `move_display_selection`, `display_scroll_start`) which the new Behavior section and expanded Display rows extend rather than replace, dock-size min/max clamp parity between keyboard and mouse, the Settings tab-cycle order changing from `Theme, Sound, Toast, Display, Integrations, Experiments` to `Theme, Sound, Toast, Display, Behavior, Integrations, Experiments` as a declared addition (not a silent reorder of existing sections), the integration-update badge tracking only `Outdated` recommendations, and every existing dock-open-from-Settings flow.
- Accepted risks for this wave (Settings presentation only; state-source and advanced-editor risks below are explicitly out of scope, owned by the two follow-on proposals): dynamic Integration/Display/Behavior rows can invalidate an index-based selection — mitigated by extending the existing typed-row-identity + normalization pattern already proven by `DisplayRowId`/`display_rows` rather than inventing a second selection model; partial bulk-integration failures can hide a late error — mitigated by target-keyed operation records with bounded summary plus scrollable detail (task 3.3); the combined UI/config span raises regression surface — mitigated by the staged migration order in `tasks.md` (geometry foundation before integration manager before native preferences) so each stage characterizes before it structurally edits.
- Roundtable outcome: zero unresolved blockers. Proceeding to 2.1 with the geometry characterization tests named above.
