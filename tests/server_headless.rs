...
fn unique_test_dir() -> PathBuf {
    let nanos = SystemTime::now()
        ...
        .unwrap_or(0);
    PathBuf::from(format!(
        "/tmp/shepherd-server-test-{}-{nanos}",
        std::process::id()
    ))
}
