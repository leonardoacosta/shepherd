...
fn spawn_shepherd(config_home: &Path, runtime_dir: &Path, socket_path: &Path) -> SpawnedShepherd {
    spawn_shepherd_with_options(config_home, runtime_dir, socket_path, None, "/bin/sh")
}

fn spawn_shepherd_with_path(
    config_home: &Path,
    runtime_dir: &Path,
    socket_path: &Path,
    path_override: Option<&Path>,
) -> SpawnedShepherd {
    spawn_shepherd_with_options(
        config_home,
        runtime_dir,
        socket_path,
        path_override,
...
#[cfg(target_os = "linux")]
fn spawn_shepherd_with_shell(
    config_home: &Path,
    runtime_dir: &Path,
    socket_path: &Path,
    shell: &str,
) -> SpawnedShepherd {
    spawn_shepherd_with_options(config_home, runtime_dir, socket_path, None, shell)
}

fn spawn_shepherd_with_options(
    config_home: &Path,
    runtime_dir: &Path,
    socket_path: &Path,
    path_override: Option<&Path>,
...
fn ping_over_socket_returns_version() {
    ...
    let child = spawn_shepherd(&config_home, &runtime_dir, &socket_path);
    ...
    cleanup_spawned_shepherd(child, base);
}
...
fn server_reload_agent_manifests_reports_runtime_override() {
    ...
    let override_dir = config_home.join("shepherd-dev").join("agent-detection");
...
fn pane_info_reports_foreground_cwd_without_changing_pane_cwd() {
    ...
    let child = spawn_shepherd_with_shell(&config_home, &runtime_dir, &socket_path, "/bin/bash");
    wait_for_socket(&socket_path, Duration::from_secs(5));
...
fn agent_start_targets_existing_pane_over_socket() {
    ...
    fs::write(
        &fake_pi,
        "#!/bin/sh\nSHEPHERD_AGENT=pi exec /bin/sleep 20\n",
    )
    .unwrap();
    fs::set_permissions(&fake_pi, fs::Permissions::from_mode(0o755)).unwrap();

    let child = spawn_shepherd_with_path(&config_home, &runtime_dir, &socket_path, Some(&bin));
    wait_for_socket(&socket_path, Duration::from_secs(5));
...
fn events_subscribe_streams_workspace_tab_and_agent_events() {
    ...
    let child = spawn_shepherd_with_path(
        &config_home,
        &runtime_dir,
        &socket_path,
...
fn pane_report_agent_updates_effective_state() {
    ...
    let session = send_request(
        &socket_path,
        &format!(
            r#"{{"id":"req_hook_session","method":"pane.report_agent_session","params":{{"pane_id":"{}","source":"shepherd:pi","agent":"pi","agent_session_path":"{}","session_start_source":"startup","seq":1}}}}"#,
            pane_id,
    ...
    let hook = send_request(
        &socket_path,
        &format!(
            r#"{{"id":"req_hook_5","method":"pane.report_agent","params":{{"pane_id":"{}","source":"shepherd:pi","agent":"pi","state":"working","message":"thinking","agent_session_path":"{}","seq":2}}}}"#,
            pane_id,
            session_path.display()
        ),
    );
    ...
    let metadata = send_request(
        &socket_path,
        &format!(
            r#"{{"id":"req_hook_metadata","method":"pane.report_metadata","params":{{"pane_id":"{}","source":"user:pi-display","agent":"pi","applies_to_source":"shepherd:pi","title":"Refactor auth","display_agent":"Pi auth","state_labels":{{"working":"deep in the mines"}},"tokens":{{"summary":"reviewing auth","model":"opus"}}}}}}"#,
            pane_id
        ),
    );
    assert_eq!(metadata["result"]["type"], "ok");
...
fn official_release_waits_for_confirmed_process_exit() {
    ...
    let session = send_request(
        &socket_path,
        &format!(
            r#"{{"id":"req_release_session","method":"pane.report_agent_session","params":{{"pane_id":"{}","source":"shepherd:pi","agent":"pi","agent_session_path":"{}","session_start_source":"startup","seq":1}}}}"#,
            pane_id,
            session_path.display()
        ),
    );
    ...
    let hook = send_request(
        &socket_path,
        &format!(
            r#"{{"id":"req_release_4","method":"pane.report_agent","params":{{"pane_id":"{}","source":"shepherd:pi","agent":"pi","state":"working","agent_session_path":"{}","seq":2}}}}"#,
            pane_id,
            session_path.display()
        ),
    );
    ...
    let released = send_request(
        &socket_path,
        &format!(
            r#"{{"id":"req_release_5","method":"pane.release_agent","params":{{"pane_id":"{}","source":"shepherd:pi","agent":"pi"}}}}"#,
            pane_id
        ),
    );
...
fn pane_clear_agent_authority_restores_fallback_state() {
    ...
    let hook = send_request(
        &socket_path,
        &format!(
            r#"{{"id":"req_clear_4","method":"pane.report_agent","params":{{"pane_id":"{}","source":"shepherd:pi","agent":"pi","state":"idle"}}}}"#,
            pane_id
        ),
    );
    assert_eq!(hook["result"]["type"], "ok");
    ...
    let cleared = send_request(
        &socket_path,
        &format!(
            r#"{{"id":"req_clear_5","method":"pane.clear_agent_authority","params":{{"pane_id":"{}","source":"shepherd:pi"}}}}"#,
            pane_id
        ),
    );
