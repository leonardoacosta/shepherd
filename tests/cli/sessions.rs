...
fn named_sessions_use_separate_servers_and_workspace_state() {
    ...
    assert!(alpha_session["socket_path"]
        .as_str()
        .unwrap()
        .ends_with("/sessions/alpha/shepherd.sock"));
...
fn integration_commands_run_locally_when_server_is_missing() {
    ...
    let expected_extension = extensions_dir.join("shepherd-agent-state.ts");
    assert!(
        !expected_extension.exists(),
        "test setup should start without extension file"
    );

    let workspace_list = Command::new(env!("CARGO_BIN_EXE_shepherd"))
        .args(["workspace", "list"])
        .env("SHEPHERD_SOCKET_PATH", &missing_socket)
        .env("HOME", &home_dir)
        .output()
        .unwrap();
    assert_eq!(workspace_list.status.code(), Some(1));

    let integration_install = Command::new(env!("CARGO_BIN_EXE_shepherd"))
    ...
    let integration_status = Command::new(env!("CARGO_BIN_EXE_shepherd"))
    ...
    let integration_uninstall = Command::new(env!("CARGO_BIN_EXE_shepherd"))
...
fn integration_status_outdated_only_prints_action_for_legacy_install() {
    ...
    fs::create_dir_all(&extensions_dir).unwrap();
    fs::write(
        extensions_dir.join("shepherd-agent-state.ts"),
        "// legacy shepherd integration\n",
    )
    .unwrap();
    ...
    assert!(stderr.contains("installed shepherd integrations need updating"));
    assert!(stderr.contains("shepherd integration install pi"));
...
fn server_stop_command_shuts_down_running_server() {
    ...
    let mut shepherd = spawn_shepherd(&config_home, &runtime_dir, &socket_path);
    wait_for_socket(&socket_path, Duration::from_secs(5));
    ...
    let pid = shepherd.child.process_id();
    let exit_status = shepherd.child.wait().unwrap();
...
fn server_stop_then_restart_restores_pane_history() {
    ...
    let mut shepherd = spawn_shepherd_with_pane_history(&config_home, &runtime_dir, &socket_path);
    wait_for_socket(&socket_path, Duration::from_secs(5));
    wait_for_socket(&client_socket, Duration::from_secs(5));
    ...
    let restarted = spawn_shepherd_with_pane_history(&config_home, &runtime_dir, &socket_path);
    wait_for_socket(&socket_path, Duration::from_secs(5));
    wait_for_socket(&client_socket, Duration::from_secs(5));
    ...
    cleanup_spawned_shepherd(restarted, base);
}
