...
fn plugin_link_works_offline_and_is_global() {
    ...
    fs::write(
        plugin_dir.join("shepherd-plugin.toml"),
        r#"
id = "example.offline"
name = "Offline Plugin"
version = "0.1.0"
...
"#,
    ...
    .unwrap();
    run_git(&source_repo, &["add", "global-plugin/shepherd-plugin.toml"]);
...
version = "0.1.0"
min_shepherd_version = "0.6.10"
description = "Apply a preferred shepherd layout"
...
[[build]]
command = ["sh", "-c", "echo built > built.txt; if [ -n \"$SHEPHERD_SESSION\" ]; then echo \"$SHEPHERD_SESSION\" > leaked-session.txt; fi"]
...
"#,
    ...
    run_git(
        &source_repo,
        &["add", "worktree-bootstrap/shepherd-plugin.toml"],
    );
    run_git(&source_repo, &["commit", "--quiet", "-m", "add plugin"]);
    ...
    fs::write(
        &git_config,
        format!(
            "[url \"file://{}\"]\n    insteadOf = https://github.com/leonardoacosta/shepherd-plugin-examples.git\n",
            source_repo.display()
        ),
    )
    .unwrap();
    ...
    let install = run_named_cli_with_env(
        ...
        &[
            ...
            "plugin",
            "install",
            "leonardoacosta/shepherd-plugin-examples/worktree-bootstrap",
            "--yes",
        ],
        &[
            ("GIT_CONFIG_GLOBAL", &git_config),
            ("SHEPHERD_SESSION", Path::new("leaked-session")),
        ],
    );
    assert!(
        install.status.success(),
    ...
    assert_eq!(plugin["source"]["owner"], "leonardoacosta");
    assert_eq!(plugin["source"]["repo"], "shepherd-plugin-examples");
    ...
    assert!(
        !managed_path
            ...
            .exists(),
        "build command should not inherit SHEPHERD_SESSION"
    );
...
"#,
    ...
    .unwrap();
    run_git(&source_repo, &["add", "build-fail/shepherd-plugin.toml"]);
    run_git(
        &source_repo,
    ...
    let install = run_named_cli_with_env(
        ...
        &[
            ...
            "plugin",
            "install",
            "leonardoacosta/shepherd-plugin-examples/build-fail",
            "--yes",
        ],
        &[("GIT_CONFIG_GLOBAL", &git_config)],
    );
...
[[build]]
command = ["definitely-missing-shepherd-build-tool-xyz"]
...
"#,
    ...
    .unwrap();
    run_git(&source_repo, &["add", "missing-tool/shepherd-plugin.toml"]);
    run_git(
        &source_repo,
    ...
    let install = run_named_cli_with_env(
        ...
        &[
            ...
            "plugin",
            "install",
            "leonardoacosta/shepherd-plugin-examples/missing-tool",
            "--yes",
        ],
        &[("GIT_CONFIG_GLOBAL", &git_config)],
    );
    ...
    assert!(stderr.contains("  build: 1/1"), "{stderr}");
    assert!(
        stderr.contains("  command: definitely-missing-shepherd-build-tool-xyz"),
        "{stderr}"
    );
...
"#,
    ...
    fs::write(
        ...
        r#"cat > shepherd-plugin.toml <<'EOF'
id = "example.manifest-mutator"
name = "Manifest Mutator"
version = "0.1.0"
min_shepherd_version = "0.0.1"
platforms = ["linux", "macos", "windows"]
...
"#,
    ...
    let install = run_named_cli_with_env(
        ...
        &[
            ...
            "plugin",
            "install",
            "leonardoacosta/shepherd-plugin-examples/manifest-mutator",
            "--yes",
        ],
        &[("GIT_CONFIG_GLOBAL", &git_config)],
    );
    ...
    let stderr = String::from_utf8_lossy(&install.stderr);
    assert!(
        stderr.contains("plugin build changed shepherd-plugin.toml after install preview"),
        "{stderr}"
    );
...
fn plugin_install_restores_previous_checkout_when_registration_fails() {
    ...
    let config_home = base.join("config");
    let runtime_dir = base.join("runtime");
    let socket_path = runtime_dir.join("fake-shepherd.sock");
    let source_repo = base.join("source-repo");
    let plugin_dir = source_repo.join("worktree-bootstrap");
    fs::create_dir_all(&plugin_dir).unwrap();
    create_committed_repo(&source_repo);
            ...
            serde_json::json!({
                ...
                "result": {
                    ...
                    "plugins": [{
                        ...
                        "version": "0.1.0",
                        "min_shepherd_version": "0.6.10",
                        "manifest_path": managed_checkout_for_server.join("shepherd-plugin.toml").display().to_string(),
                        "plugin_root": managed_checkout_for_server.display().to_string(),
                        "enabled": true,
                        "source": {
                            "kind": "github",
                            "owner": "leonardoacosta",
                            "repo": "shepherd-plugin-examples",
