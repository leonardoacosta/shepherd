...
fn help_commands_exit_successfully() {
    ...
    for args in help_cases {
        ...
        assert!(
            ...
            "shepherd {} failed: status={:?} stdout={} stderr={}",
            args.join(" "),
            output.status.code(),
...
fn subcommand_help_explains_automation_semantics_without_a_server() {
    ...
    for (args, expected) in cases {
        ...
        assert!(
            ...
            "shepherd {} help did not contain {expected:?}: {stdout}",
            args.join(" ")
        );
    }
}
...
fn removed_wait_and_agent_send_commands_are_rejected() {
    let wait = Command::new(env!("CARGO_BIN_EXE_shepherd"))
    ...
    let help = Command::new(env!("CARGO_BIN_EXE_shepherd"))
        .arg("--help")
        .output()
        .unwrap();
    assert!(!String::from_utf8_lossy(&help.stdout).contains("shepherd wait <subcommand>"));

    let send = Command::new(env!("CARGO_BIN_EXE_shepherd"))
        .args(["agent", "send", "reviewer", "hello"])
        .output()
        .unwrap();
    assert_eq!(send.status.code(), Some(2));
    let stderr = String::from_utf8_lossy(&send.stderr);
    assert!(stderr.contains("shepherd agent send-keys"));
    assert!(!stderr.contains("shepherd agent send <"));
}
...
fn agent_cli_rejects_invalid_wait_and_rename_grammar_locally() {
    for args in [
        ...
        let output = Command::new(env!("CARGO_BIN_EXE_shepherd"))
            .args(args)
            .env("SHEPHERD_SOCKET_PATH", "/nonexistent/shepherd.sock")
            .output()
            .unwrap();
        assert_eq!(
            output.status.code(),
            Some(2),
            "shepherd {}: stdout={} stderr={}",
            args.join(" "),
            String::from_utf8_lossy(&output.stdout),
            String::from_utf8_lossy(&output.stderr)
        );
...
fn completion_command_prints_zsh_script_without_session_startup() {
    ...
    let stdout = String::from_utf8_lossy(&output.stdout);
    assert!(stdout.contains("#compdef shepherd"), "stdout: {stdout}");
    assert!(
        !stdout.contains("--cwd=[]"),
...
fn root_help_hides_explicit_client_command() {
    ...
    let stdout = String::from_utf8_lossy(&output.stdout);
    assert!(
        !stdout.contains("shepherd client"),
...
fn root_help_advertises_api_schema_command_group() {
    ...
    let stdout = String::from_utf8_lossy(&output.stdout);
    assert!(
        stdout.contains("shepherd api <subcommand>"),
...
fn api_schema_default_output_is_a_short_summary() {
    ...
    assert!(output.status.success());
    let stdout = String::from_utf8_lossy(&output.stdout);
    assert!(stdout.contains("Shepherd API schema"), "stdout: {stdout}");
    assert!(
        stdout.contains("Use `shepherd api schema --json`"),
        "stdout: {stdout}"
    );
    assert!(
        stdout.len() < 400,
...
fn api_schema_output_writes_bundled_schema_to_file() {
    let base = unique_test_dir();
    fs::create_dir_all(&base).unwrap();
    let schema_path = base.join("shepherd-api.schema.json");
...
fn explicit_client_command_respects_nested_guard() {
    ...
    let output = Command::new(env!("CARGO_BIN_EXE_shepherd"))
        ...
        .env_remove("SHEPHERD_CONFIG_PATH")
        .output()
        .unwrap();
...
fn removed_show_changelog_flag_fails_before_nested_guard() {
    ...
    assert!(
        !stderr.contains("nested shepherd"),
