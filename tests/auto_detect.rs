...
use support::{
    ...
    unregister_spawned_shepherd_pid,
};
...
fn unique_test_dir() -> PathBuf {
    let nanos = SystemTime::now()
        ...
        .unwrap_or(0);
    PathBuf::from(format!(
        "/tmp/shepherd-autodetect-test-{}-{nanos}",
        std::process::id()
    ))
}
...
fn cleanup_spawned_shepherd(spawned: SpawnedShepherd, base: PathBuf) {
    drop(spawned);
...
fn spawn_server(
    ...
    cmd.env("SHEPHERD_SOCKET_PATH", api_socket_path);
...
/// Spawn `shepherd` (no subcommand) — the auto-detect launch path.
fn spawn_shepherd_auto(
    config_home: &Path,
    runtime_dir: &Path,
    api_socket_path: &Path,
    _client_socket_path: &Path,
...
/// Spawn `shepherd --no-session` — the monolithic escape hatch.
fn spawn_shepherd_no_session(
    config_home: &Path,
    runtime_dir: &Path,
    api_socket_path: &Path,
) -> SpawnedShepherd {
...
/// Running `shepherd` with no server present starts a server
...
fn auto_detect_no_server_spawns_server_and_attaches() {
    ...
    // Run `shepherd` (no subcommand) — should auto-detect, spawn server, attach as client.
    let shepherd = spawn_shepherd_auto(&config_home, &runtime_dir, &api_socket, &client_socket);
    ...
    let client_pid = shepherd.child.process_id().expect("client should have PID");
    assert!(
...
/// Running `shepherd` with a server already running attaches
...
fn auto_detect_server_running_attaches_directly() {
    ...
    // Run `shepherd` (no subcommand) — should detect the running server and attach.
    let client = spawn_shepherd_auto(&config_home, &runtime_dir, &api_socket, &client_socket);
    ...
    cleanup_spawned_shepherd(client, PathBuf::from("/nonexistent"));
    cleanup_spawned_shepherd(server, base);
}

/// Socket path resolution is consistent between server and client.
/// Both derive the client socket from the `SHEPHERD_SOCKET_PATH` override,
...
fn auto_detect_socket_path_consistency() {
    ...
    // Run `shepherd` with custom socket paths.
    let shepherd = spawn_shepherd_auto(&config_home, &runtime_dir, &api_socket, &client_socket);
...
/// `shepherd --no-session` bypasses server/client and runs
...
fn no_session_flag_runs_monolithically() {
    ...
    // Run `shepherd --no-session` — monolithic mode, no server/client.
    let shepherd = spawn_shepherd_no_session(&config_home, &runtime_dir, &api_socket);
    ...
    // there is only one shepherd process.
    let client_pid = shepherd.child.process_id().expect("should have PID");
    assert!(
        process_exists(client_pid),
...
fn cli_subcommands_work_through_server() {
    ...
    // Test `shepherd workspace list` through the server's API socket.
    ...
    // Test `shepherd pane list` through the server's API socket.
...
/// persists after the client exits, and a new `shepherd` can reattach.
#[test]
fn auto_detect_server_persists_and_reattaches() {
    ...
    // Run `shepherd` — auto-detect spawns server + attaches client.
    let mut client1 = spawn_shepherd_auto(&config_home, &runtime_dir, &api_socket, &client_socket);
    wait_for_socket(&api_socket, Duration::from_secs(10));
    wait_for_socket(&client_socket, Duration::from_secs(10));
    ...
    // Run `shepherd` again — should detect the running server and reattach.
    let client2 = spawn_shepherd_auto(&config_home, &runtime_dir, &api_socket, &client_socket);
    thread::sleep(Duration::from_millis(500));
    ...
    cleanup_spawned_shepherd(client2, PathBuf::from("/nonexistent"));
    cleanup_test_base(&base);
}
...
fn auto_detect_default_socket_path_from_config_dir() {
    ...
    // Don't set SHEPHERD_SOCKET_PATH or SHEPHERD_CLIENT_SOCKET_PATH.
    ...
    let api_socket = config_home.join(app_dir_name).join("shepherd.sock");
    let client_socket = config_home.join(app_dir_name).join("shepherd-client.sock");
...
fn auto_detect_writes_client_and_server_logs_to_separate_files() {
    ...
    let spawned = spawn_shepherd_auto(&config_home, &runtime_dir, &api_socket, &client_socket);
    wait_for_socket(&api_socket, Duration::from_secs(10));
    wait_for_socket(&client_socket, Duration::from_secs(10));
    ...
    let client_log = log_dir.join("shepherd-client.log");
    let server_log = log_dir.join("shepherd-server.log");
    let monolith_log = log_dir.join("shepherd.log");
    ...
    assert!(
        ...
        "persistent client logs should not land in shepherd.log: {monolith_content}"
    );

    cleanup_spawned_shepherd(spawned, base);
}
...
fn no_session_writes_startup_logs_to_monolith_file() {
    ...
    let spawned = spawn_shepherd_no_session(&config_home, &runtime_dir, &api_socket);
    wait_for_socket(&api_socket, Duration::from_secs(10));
...
fn auto_detect_respects_nested_guard_before_auto_attach() {
    ...
    let output = Command::new(env!("CARGO_BIN_EXE_shepherd"))
        ...
        .env("SHEPHERD_SOCKET_PATH", &api_socket)
    ...
    assert!(
        stderr.contains("nested shepherd is disabled by default"),
