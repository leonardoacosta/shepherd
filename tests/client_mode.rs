...
use support::{
    ...
    register_runtime_dir, register_spawned_shepherd_pid, unregister_spawned_shepherd_pid,
...
fn unique_test_dir() -> PathBuf {
    let nanos = SystemTime::now()
        ...
        .unwrap_or(0);
    PathBuf::from(format!(
        "/tmp/shepherd-client-test-{}-{nanos}",
        std::process::id()
    ))
}
...
fn spawn_client_process(
    ...
    cmd.arg("client");
    cmd.env("SHEPHERD_DISABLE_SOUND", "1");
        ...
        match read_server_message(&mut stream) {
            Ok((1, payload)) => {
                ...
                if last_frame_text.contains("config.toml")
                    && last_frame_text.contains("shepherd config check")
                {
...
fn server_unreachable_shows_clear_error() {
    ...
    let output = std::process::Command::new(env!("CARGO_BIN_EXE_shepherd"))
        .arg("client")
        .env("SHEPHERD_DISABLE_SOUND", "1")
    ...
    assert!(
        stderr.contains("Is shepherd server running?"),
        ...
        while Instant::now() < deadline {
            match thin_reader.read(&mut buf) {
                Ok(n) if n > 0 => {
                    ...
                    if output.to_lowercase().contains("shepherd:") {
                        break;
                    }
                }
...
fn client_receives_notify_on_agent_state_change() {
    ...
    // Full-suite runs can queue additional frame traffic before the notify.
    stream
